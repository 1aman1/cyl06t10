...
#define DEBUG true
/*
has two pointers instead of one i.e.,
Head & Tail, both.

which reducces end of the list operation complexity to O(1) from O(N)
...
bool linked_list::insert(int value, int index)
{
  ...
  else
  {
    ...
    if (index == size() + 1)
    {
      DEBUG == true ? std::cout << "using tail" << std::endl : std::cout << "";
